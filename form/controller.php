<?php
    ...
    if (isset($_POST['signup'])) {
        ...
        // count erros
        if (count($errors) === 0) {
            $insertQuery = "INSERT INTO form (firstName,lastName,email,password,gender,code,status)
            ...
            if ($insertInfo) {
                ...
                $sender = 'From: usercrazy34@gmail.com';
    ...
    if (isset($_POST['verify'])) {
        ...
        $otp_query = "SELECT * FROM form WHERE code = $otp";
        ...
        if (mysqli_num_rows($otp_result) > 0) {
            ...
            $update_query = "UPDATE form SET status = '$update_status' , code = $update_code WHERE code = $fetch_code";
    ...
    if (isset($_POST['login'])) {
        ...
        if (mysqli_num_rows($email_check) > 0) {
            $passwordQuery = "SELECT * FROM form WHERE email = '$email' AND password = '$password'";
    ...
    if (isset($_POST['forgot_password'])) {
        ...
        $emailCheckQuery = "SELECT * FROM form WHERE email = '$email'";
        ...
        if ($emailCheckResult) {
            ...
            if (mysqli_num_rows($emailCheckResult) > 0) {
                $code = rand(999999, 111111);
                $updateQuery = "UPDATE form SET code = $code WHERE email = '$email'";
...
if(isset($_POST['verifyEmail'])){
    ...
    $verifyQuery = "SELECT * FROM form WHERE code = $OTPverify";
    $runVerifyQuery = mysqli_query($conn, $verifyQuery);
    if($runVerifyQuery){
        if(mysqli_num_rows($runVerifyQuery) > 0){
            $newQuery = "UPDATE form SET code = 0";
...
if(isset($_POST['changePassword'])){
    ...
    if (strlen($_POST['password']) < 8) {
        ...
        if ($_POST['password'] != $_POST['confirmPassword']) {
            ...
            $updatePassword = "UPDATE form SET password = '$password' WHERE email = '$email'";
